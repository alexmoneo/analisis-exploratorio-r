...
##Introducción
Dado que no tengo muy claro que puedo sacar con el dataset elegido, he decidio comprobar en el dataset en el que estan los mismos estudiantes como influye los factores familiares a la nota de los estudiantes, en un principio habia elegido hacerlo con los dos dataset, pero debido a la limitacion de hojas para la entrega he decidido utilizar solo un dataset.
```
```

## Carga de los datos
Debemos descargar los datos desde una url, para tener mas estructurado el proyecto creare un directorio de datos donde guardare los dataset descargados. Una ves descardo los archivos generare los data frame para los estudiantes de Matematicas y Portugues, luego generare uno mezclando ambos y cogiendo solo los estudiantes repetidos. Despues creare las varibles que necesite.
```{r message=FALSE}
...
kable(df, caption = "Estudiantes de matematicas y portugues")
...
Ahora vamos a intentar relacionar las variables para ver si alguna influye sobre la nota final, solo estoy usando la variable x(matematicas para las pruebas)
```{r message=FALSE}
...
##Conclusion
Despues de ver las graficas de correlacion parace que no hay muchar realacion entre las variables de la familia y la nota final, de todas formas es muy posible que alguien que se dedique a la estadistica pueda interpretar mucho  mejor que yo estos datos.
```{r message=FALSE}
